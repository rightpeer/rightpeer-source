...
def get_options():
    mc_path_default = str(Path.home() / "rightpeer")
    ...
    parser.add_argument("-m", "--rightpeer", metavar="DIR", default=mc_path_default,
                        help="RightPeer path prefix (default: %(default)s)")
    ...
    if not Path(options.rightpeer).exists():
        parser.error("{!r}: RightPeer path does not exist".format(options.rightpeer))

    logger.info("{} - {}".format(logger.name, parser.description))
    logger.info("  Path:     {}".format(options.rightpeer))
...
def main():
    ...
    options = get_options()
    os.chdir(str(Path(options.rightpeer) / "v8build" / "v8" / "out.gn" / "x64.release"))
