...
Value liststreamtxitems(const Array& params, bool fHelp)
{
    ...
    if((mc_gState->m_WalletMode & MC_WMD_TXS) == 0)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "API is not supported with this wallet version. For full streams functionality, run \"rightpeerd -walletdbversion=2 -rescan\" ");        
    }   
