    ...
    BOOST_FOREACH(const CTxIn& txin, wtx.vin)
    {
        ...
        if (prevout_found)
        {
            if(IsTxOutMineAndInList(pwalletMain,prevout,addresses,filter))
            {
                ...
                if(ParseRightpeerTxOutToBuffer(txin.prevout.hash,prevout,amounts,lpScript,NULL,NULL,strFailReason))
                {
    ...
    for (int i = 0; i < (int)wtx.vout.size(); ++i)
    {
        ...
        if(!txout.scriptPubKey.IsUnspendable())
        {
            ...
            if(IsTxOutMineAndInList(pwalletMain,txout,addresses,filter))
            {
                fIsMine=true;
                nIsToMeCount++;
                ParseRightpeerTxOutToBuffer(wtx.GetHash(),txout,amounts,lpScript,NULL,&required,strFailReason);
