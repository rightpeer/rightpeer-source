...
//            if( AvailableFromStatus(retrieve_status) && ((retrieve_status & MC_OST_STORAGE_MASK) == MC_OST_OFF_CHAIN ))
            if( AvailableFromStatus(retrieve_status) )
            {
                ...
                {        
                    ...
                    if(pRightPeerFilterEngine->RunStreamFilters(wtx,stream_output,(unsigned char *)stream_id,filter_block,filter_offset, 
...
//    int iad=-1;
    ...
    if(fIsFound)
    {
        ...
        int required=0;
        if(ParseRightpeerTxOutToBuffer(hash,txout,amounts,lpScript,NULL,&required,strFailReason))
        {
                              ...
                              string *strError)
{
    ...
    {
        *strError="Offchain data is not supported with this wallet version. To get this functionality, run \"rightpeerd -walletdbversion=2 -rescan\"";
