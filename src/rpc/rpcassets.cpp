    ...
    if (params.size() > 0)
    {
        if (params[0].get_str() != "*") 
        {
            if (params[0].get_str() != "") 
            {
                ...
                {
                    throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need getassetbalances, run rightpeerd -walletdbversion=1 -rescan, but the wallet will perform worse");        
                }            
            }
            BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
            {
...
Value getfilterassetbalances(const Array& params, bool fHelp)
{
    ...
    for(int j=0;j<(int)pRightPeerFilterEngine->m_Tx.vin.size();j++)
    {
        int n=pRightPeerFilterEngine->m_Tx.vin[j].prevout.n;
        CCoins coins;
        {
            LOCK(mempool.cs);
            CCoinsViewMemPool view(pcoinsTip, mempool);
            if (!view.GetCoins(pRightPeerFilterEngine->m_Tx.vin[j].prevout.hash, coins))
            {
                return Value::null;                                                                 
            }
                ...
                if(CreateAssetBalanceList(coins.vout[n],asset_amounts,lpScript,&required))
                {
                    for(int i=0;i<asset_amounts->GetCount();i++)
                    {
                        ...
                        else
                        {
                            ...
                            if(memcmp(lpAsset->GetTxID(),&(pRightPeerFilterEngine->m_Tx.vin[gin].prevout.hash),sizeof(uint256)) == 0)
                            {
    ...
    for (int j = 0; j < (int)pRightPeerFilterEngine->m_Tx.vout.size(); ++j)
    {
        const CTxOut& txout = pRightPeerFilterEngine->m_Tx.vout[j];
        if(!txout.scriptPubKey.IsUnspendable())
        {
            CTxDestination address;
            int required=0;
            ...
            if(lpAsset)
            {
                ...
                if(CreateAssetBalanceList(txout,asset_amounts,lpScript,&required))
                {
                    for(int i=0;i<asset_amounts->GetCount();i++)
                    {
                        ...
                        {
                            if(memcmp(lpAsset->GetTxID(),&(pRightPeerFilterEngine->m_TxID),sizeof(uint256)) == 0)
                            {
                                if( mc_GetABRefType(asset_amounts->GetRow(i)) == MC_AST_ASSET_REF_TYPE_GENESIS )                    
                                {
                                    quantity=mc_GetABQuantity(asset_amounts->GetRow(i));
