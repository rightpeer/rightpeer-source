...
using namespace std;
void PushRightPeerRelay(CNode* pto, uint32_t msg_type,vector<CAddress>& path,vector<CAddress>& path_to_follow,vector<unsigned char>& payload);
...
//                    PushRightPeerRelay(pnode, MC_RMT_GLOBAL_PING,path,path_to_follow,payload);
...
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolRightpeer())
    {
        obj.push_back(Pair("subversion",
            FormatSubVersion("RightPeer", mc_gState->GetProtocolVersion(), std::vector<string>())));
    }
    else
    {
        obj.push_back(Pair("subversion",
