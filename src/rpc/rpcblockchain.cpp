...
/* MCHN START */
bool ParseRightpeerTxOutToBuffer(uint256 hash,const CTxOut& txout,mc_Buffer *amounts,mc_Script *lpScript,int *allowed,int *required,string& strFailReason);
...
//        throw JSONRPCError(RPC_INVALID_PARAMS, "Wrong number of parameters");                    
    ...
    if( (vin < 0) || (vin >= (unsigned int)pRightPeerFilterEngine->m_Tx.vin.size()) )
    {
    ...
    Array getxoutparams;
    getxoutparams.push_back(pRightPeerFilterEngine->m_Tx.vin[vin].prevout.hash.ToString());
    getxoutparams.push_back((int64_t)pRightPeerFilterEngine->m_Tx.vin[vin].prevout.n);
...
Value gettxout(const Array& params, bool fHelp)
{
    ...
    if (fMempool) {
        ...
        if (!view.GetCoins(hash, coins))
            return Value::null;
        if(pRightPeerFilterEngine->m_TxID == 0)                                // In filter we already checked this input exists, but mempool is dirty
        {
