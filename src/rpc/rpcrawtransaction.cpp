 ...
 */ 
    ...
    {
        if(pRightPeerFilterEngine->m_TxID != 0)
        {
...
/* MCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be AccepRightPeerTransaction
...
int VerifyNewTxForStreamFilters(const CTransaction& tx,std::string &strResult,mc_RightPeerFilter **lppFilter,int *applied)            
{
    ...
    if(pRightPeerFilterEngine->NoStreamFilters())
    {
        return MC_ERR_NOERROR;
    }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) 
    {
        ...
        if(result.type() == obj_type)
        {
            ...
            int err=pRightPeerFilterEngine->RunStreamFilters(tx,i,(unsigned char*)&hash+MC_AST_SHORT_TXID_OFFSET,-1,0, 
...
//        throw runtime_error("Help message not found\n");
    ...
    if(pRightPeerFilterEngine->m_Vout < 0)
    {
    ...
    if(pRightPeerFilterEngine->m_Vout >= (int)pRightPeerFilterEngine->m_Tx.vout.size())
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "vout out of range");                                            
    }
    Value result=DataItemEntry(pRightPeerFilterEngine->m_Tx,pRightPeerFilterEngine->m_Vout,streams_already_seen, 0x0E00);
...
//    result.get_obj().push_back(Pair("txid", pRightPeerFilterEngine->m_Tx.GetHash().GetHex()));
    result.get_obj().push_back(Pair("vout", pRightPeerFilterEngine->m_Vout));
...
/*    
    ...
    if(pRightPeerFilterEngine->m_TxID != 0)
    {
        ...
        tx=pRightPeerFilterEngine->m_Tx;
    }
    else
    {
        if(params.size() == 0)
...
*/    
    Object result;
    TxToJSON(pRightPeerFilterEngine->m_Tx, 0, result);
...
Value appendrawchange(const Array& params, bool fHelp)
{
    ...
    {
        if(ParseRightpeerTxOutToBuffer(0,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
    ...
    {
        if(ParseRightpeerTxOutToBuffer(tx.vin[i].prevout.hash,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
            nAmount+=txout.nValue;
        }
        i++;
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=rightpeer");
    }    
