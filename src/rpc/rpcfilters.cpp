...
Value createfilterfromcmd(const Array& params, bool fHelp)
{
    ...
    err=pFilterEngine->CreateFilter(js.c_str(),filter_main_name,pRightPeerFilterEngine->m_CallbackNames[filter_type],worker,strError);
    delete worker;
    if(err)
    {
...
Value listfilters(const Array& params, uint32_t filter_type)
{
    ...
    for(int i=0;i<(int)pRightPeerFilterEngine->m_Filters.size();i++)
    {
        Object entry;

        if((filter_list.size() == 0) || 
           (filter_list.find(*(uint256*)pRightPeerFilterEngine->m_Filters[i].m_Details.GetTxID()) != filter_list.end()) )
        {
            entry=FilterEntry(pRightPeerFilterEngine->m_Filters[i].m_Details.GetTxID(),output_level,filter_type);
            if(entry.size()>0)
            {
                bool take_it=false;
                ...
                if(take_it)
                {
                    bool valid=pRightPeerFilterEngine->m_Filters[i].m_CreateError.size() == 0;
                    entry.push_back(Pair("compiled",valid));
                    if(check_approval)
                    {
                        entry.push_back(Pair("approved",mc_gState->m_Permissions->FilterApproved(lpEntity,&(pRightPeerFilterEngine->m_Filters[i].m_FilterAddress)) !=0 ));
                        entry.push_back(Pair("address",pRightPeerFilterEngine->m_Filters[i].m_FilterAddress.ToString()));
                    }
...
Value getfiltertxid(const Array& params, bool fHelp)
{
    ...
    return pRightPeerFilterEngine->m_TxID.ToString();
}
...
Value setfilterparam(const json_spirit::Array& params, bool fHelp)
{
    ...
    if( (param_name == "maxshowndata") )
    {
        if( (params[1].type() == int_type) || (params[1].type() == str_type) )
        {
            ...
            pRightPeerFilterEngine->m_Params.m_MaxShownData=nValue;
        }
        else
        {
...
Value testfilter(const vector <uint160>& entities,const  char *filter_code, Value txparam,int vout_in,uint32_t filter_type)
{
    ...
    mc_Filter *worker=new mc_Filter;
//    err=pFilterEngine->CreateFilter(filter_code,filter_main_name,pRightPeerFilterEngine->m_CallbackNames[filter_type],worker,strError);
    err=pFilterEngine->CreateFilter(filter_code,filter_main_name,pRightPeerFilterEngine->m_CallbackNames[filter_type],worker,pRightPeerFilterEngine->GetSendTimeout(), strError);
    if(err)
    {
    ...
    if(txhex.size())
    {
        ...
        if(relevant_filter)
        {
            err=pRightPeerFilterEngine->RunFilterWithCallbackLog(tx,vout,worker,strError,callbacks);
            if(err)
            {
                errorCode=RPC_INTERNAL_ERROR;
