...
uint64_t nLocalRightPeerServices = 0;
...
/* MCHN START */
    ...
    nLocalRightPeerServices |= pEF->NET_Services();
    if(mc_gState->m_NetworkParams->IsProtocolRightpeer())
    {
        subver=FormatSubVersion("RightPeer", mc_gState->GetProtocolVersion(), std::vector<string>());
    }
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
                nLocalHostNonce, subver, nBestHeight, true, nLocalRightPeerServices);
/* MCHN END */
}
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "RightPeer " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. RightPeer Core is probably already running."), addrBind.ToString());
        else
