...
int mc_OneRightpeerParam::IsRelevant(int version)
{
    int ret=1;
...
void mc_RightpeerParams::Destroy()
{
    if(m_lpData)
    {
        mc_Delete(m_lpData);
...
int64_t mc_RightpeerParams::GetInt64Param(const char *param)
{
    int size;
...
double mc_RightpeerParams::Int64ToDecimal(int64_t value)
{
    if(value < 0)
    {
...
int64_t mc_RightpeerParams::DecimalToInt64(double value)
{
    return (int64_t)(value*MC_PRM_DECIMAL_GRANULARITY+mc_gState->m_NetworkParams->ParamAccuracy());    
}

int mc_RightpeerParams::GetParamFromScript(char* script,int64_t *value,int *size)
{
    char *ptr;    
    ptr=script;
    ptr+=strlen(ptr)+1;
...
double mc_RightpeerParams::GetDoubleParam(const char *param)
{
...
void* mc_RightpeerParams::GetParam(const char *param,int* size)
{
    if(m_lpIndex == NULL)
    {
        return NULL;
...
int mc_RightpeerParams::IsParamUpgradeValueInRange(const mc_OneRightpeerParam *param,int version,int64_t value)
{
...
int mc_RightpeerParams::CanBeUpgradedByVersion(const char *param,int version,int size)
{
    if(m_lpIndex == NULL)
    {
...
int mc_RightpeerParams::SetParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc_RightpeerParams::SetParam(const char *param,int64_t value)
{
    int size;
    char buf[8];
    if(m_lpIndex == NULL)
...
void mc_RightpeerParams::Init()
{
    ...
    m_Count=sizeof(RightpeerParamArray)/sizeof(mc_OneRightpeerParam);
    max_size=0;
    
    for(i=0;i<m_Count;i++)
    {
        size=0;
        switch((RightpeerParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
        {
            case MC_PRM_BINARY  : size=(RightpeerParamArray+i)->m_MaxStringSize;   break;
            case MC_PRM_STRING  : size=(RightpeerParamArray+i)->m_MaxStringSize+1; break;
    ...
    m_lpParams=(mc_OneRightpeerParam*)mc_New(sizeof(RightpeerParamArray));
    m_lpData=(char*)mc_New(max_size);
    m_lpCoord=(int*)mc_New(2*m_Count*sizeof(int));
    
    memcpy(m_lpParams,RightpeerParamArray,sizeof(RightpeerParamArray));
    for(i=0;i<m_Count;i++)
    {
...
int mc_RightpeerParams::Create(const char* name,int version)
{
    int size,offset,i,set;
    mc_OneRightpeerParam *param;
    char *ptrData;
    int num_sets;
...
double mc_RightpeerParams::ParamAccuracy()
{
    return 1./(double)MC_PRM_DECIMAL_GRANULARITY;
}


const mc_OneRightpeerParam *mc_RightpeerParams::FindParam(const char* param)
{
    int i;
    for(i=0;i<m_Count;i++)
    {
        if(strcmp((RightpeerParamArray+i)->m_Name,param) == 0)
        {
            return RightpeerParamArray+i;
        }        
    }    
    return NULL;
}

int mc_RightpeerParams::Read(const char* name)
{
    return Read(name,0,NULL,0);
}

int mc_RightpeerParams::Read(const char* name,int argc, char* argv[],int create_version)
{
    ...
    mc_RightpeerParams *lpDefaultParams;
    ...
    lpDefaultParams = new mc_RightpeerParams;
    ...
    for(i=0;i<m_Count;i++)
    {
        ...
        {        
            ...
            switch((RightpeerParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
            {
                ...
                case MC_PRM_INT32:
                    size=4;
                    if(((RightpeerParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) == MC_PRM_USER)
                    {
                        if(atoll(ptr) > (RightpeerParamArray+i)->m_MaxIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too high: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                        if(atoll(ptr) < (RightpeerParamArray+i)->m_MinIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too low: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                    }
                    if((RightpeerParamArray+i)->m_Type & MC_PRM_DECIMAL)
                    {
                        double d=atof(ptr);
                        if(d >= 0)
                        {
        ...
        {
            if( ((((RightpeerParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_CALCULATED) && 
               (((RightpeerParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_GENERATED)) ||
               (argc > 0) )     
            {
                strcpy(m_lpData+offset,param->m_Name);
                offset+=strlen(param->m_Name)+1;
...
int mc_RightpeerParams::Set(const char *name,const char *source,int source_size)
{
...
int mc_RightpeerParams::Clone(const char* name, mc_RightpeerParams* source)
{
    int err;
...
int mc_RightpeerParams::CalculateHash(unsigned char *hash)
{
    int i;
    int take_it;
    mc_SHA256 *hasher;
...
int mc_RightpeerParams::Validate()
{
    int i,size,offset;
    int isGenerated;
    int isMinimal;
    ...
    if(isValid)
    {
        ...
        else
        {
            ...
            if(strcmp((char*)protocol_name,"rightpeer") == 0)
            {
...
int mc_RightpeerParams::Print(FILE* fileHan)
{
    int i,c,size;
    int version;
    int header_printed;
    ...
    fprintf(fileHan,"# ==== RightPeer configuration file ====\n\n");
    fprintf(fileHan,"# Created by rightpeer-util \n");
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"rightpeerd %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun rightpeer-util. \n");
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"rightpeerd %s\".\n",Name());
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"rightpeerd %s\".\n",Name());
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                ...
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence rightpeer network configuration. \n");
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"rightpeer-util clone %s <new-network-name>\". \n",Name());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running rightpeerd for this chain. \n");                                    
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"rightpeer-util clone %s <new-network-name>\" to generate new network. \n",Name());
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by rightpeer-util.\n");
                            fprintf(fileHan,"# They SHOULD ONLY BE EDITED IF YOU KNOW WHAT YOU ARE DOING. \n");
                            break;
                        case MC_PRM_CALCULATED:
                            fprintf(fileHan,"# The following parameters were generated by rightpeerd.\n");
...
int mc_RightpeerParams::Write(int overwrite)
{
    FILE *fileHan;
    int create;
...
const char* mc_RightpeerParams::Name()
{
    return (char*)GetParam("chainname",NULL);
}

const unsigned char* mc_RightpeerParams::MessageStart()
{
    return (unsigned char*)GetParam("networkmessagestart",NULL);
}

const unsigned char* mc_RightpeerParams::DefaultMessageStart()
{
    return c_DefaultMessageStart;
}


const unsigned char* mc_RightpeerParams::AddressVersion()
{
    return (unsigned char*)GetParam("addresspubkeyhashversion",NULL);
}

const unsigned char* mc_RightpeerParams::AddressScriptVersion()
{
    return (unsigned char*)GetParam("addressscripthashversion",NULL);
}

const unsigned char* mc_RightpeerParams::AddressCheckumValue()
{
    return (unsigned char*)GetParam("addresschecksumvalue",NULL);
}


int mc_RightpeerParams::ProtocolVersion()
{
...
int mc_RightpeerParams::IsProtocolRightpeer()
{
    return m_IsProtocolRightPeer;
}
