...
/* MCHN START */
static Checkpoints::MapCheckpoints mapCheckpointsRightpeer =
        boost::assign::map_list_of
        ( -1, uint256("0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"))
        ;
static const Checkpoints::CCheckpointData dataRightpeer = {
        &mapCheckpointsRightpeer,
        0,
        0,
        0
    };
...
class CRightPeerParams : public CMainParams {
public:
    CRightPeerParams() {    };

    void SetFixedParams(const char *NetworkName)
    {
        networkID = CBaseChainParams::RIGHTPEER;
...
//        convertSeed6(vFixedSeeds, pnSeed6_test, ARRAYLEN(pnSeed6_test));
    ...
    void SetRightPeerParam(const char*param_name,int64_t value)
    {
    ...
    void SetRightPeerRuntimeParams()
    {
...
//        assert(hashGenesisBlock == uint256("0x0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"));
    ...
    const Checkpoints::CCheckpointData& Checkpoints() const
    {
        return dataRightpeer;
    }
};
static CRightPeerParams rightPeerParams;


bool SelectRightPeerParams(const char *NetworkName)
{
    SelectRightPeerBaseParams(NetworkName,(int)mc_gState->m_NetworkParams->GetInt64Param("defaultrpcport"));

    rightPeerParams.SetFixedParams(NetworkName);

    rightPeerParams.Initialize();

    pCurrentParams = &rightPeerParams;
...
void SetRightPeerParams()
{
    rightPeerParams.SetRightPeerParams();
}

void SetRightPeerParam(const char*param_name,int64_t value)
{
    rightPeerParams.SetRightPeerParam(param_name,value);
}


void SetRightPeerRuntimeParams()
{
    rightPeerParams.SetRightPeerRuntimeParams();
}

bool InitializeRightPeerParams()
{

    rightPeerParams.Initialize();

    rightPeerParams.SetGenesis();
