...
int mc_GenerateConfFiles(const char *network_name)
{
    ...
    fileHan=mc_OpenFile(NULL,"rightpeer",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(NULL,"rightpeer",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
    }
    if(fileHan)
    {
    ...
    fileHan=mc_OpenFile(network_name,"rightpeer",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(network_name,"rightpeer",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
        if(fileHan)
        {
            fprintf(fileHan,"rpcuser=rightpeerrpc\n");
...
int mc_RightPeerParams::Build(const unsigned char* pubkey, int pubkey_size) 
{
