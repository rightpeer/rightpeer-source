...
#ifndef RIGHTPEERPARAMS_H
#define	RIGHTPEERPARAMS_H
...
typedef struct mc_OneRightpeerParam
{    
...
} mc_OneRightpeerParam;

typedef struct mc_RightpeerParams
{    
    char *m_lpData;
    mc_MapStringIndex *m_lpIndex;
    mc_OneRightpeerParam *m_lpParams;
    int *m_lpCoord;
    int m_Status;
    int m_Size;
    int m_Count;
    int m_IsProtocolRightPeer;
    int m_ProtocolVersion;
    ...
    int Clone(const char *name,mc_RightpeerParams *source);
    ...
    const mc_OneRightpeerParam *FindParam(const char *param);
    ...
    int IsParamUpgradeValueInRange(const mc_OneRightpeerParam *param,int version,int64_t value);
