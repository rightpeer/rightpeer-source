...
string CWallet::SetDefaultKeyIfInvalid(std::string init_privkey)
{
    ...
    if(!vchSecret.SetString(init_privkey))
    {
        ...
        {
            return "Private key version is not set, please chose seed node running at least RightPeer 1.0 beta 2";            
        }
                                ...
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse, int *eErrorCode)
{
    ...
    return CreateRightPeerTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, addresses, min_conf, min_inputs, max_inputs, lpCoinsToUse, 
                                ...
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse, int *eErrorCode)
{
    ...
    return CreateRightPeerTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, addresses, min_conf, min_inputs, max_inputs,lpCoinsToUse,eErrorCode);
}

int CWallet::SelectRightPeerCombineCoinsMinConf(int nConfMine, int nConfTheirs, vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
...
bool CWallet::SelectRightPeerCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
...
{
//    printf("SelectRightPeerCoinsMinConf\n");
    setCoinsRet.clear();
    nValueRet = 0;
...
bool CWallet::SelectRightPeerCoins(const CAmount& nTargetValue, vector<COutput> &vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts, 
                                int in_selected_row,int in_asset_row,int in_preferred_row,
...
//                    if(mc_GetLE(in_amounts->GetRow(in_selected_row)+coin_id*MC_AST_ASSET_REF_SIZE,MC_AST_ASSET_QUANTITY_SIZE) == 0)// BUG??? 
    ...
    {
        if((SelectRightPeerCoinsMinConf(nTargetValue, 1, 6, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            SelectRightPeerCoinsMinConf(nTargetValue, 1, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            (bSpendZeroConfChange && SelectRightPeerCoinsMinConf(nTargetValue, 0, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet))))
        {
            return true;
        }
...
{
    {
        ...
        int64_t nKeys = max(GetArg("-keypool", mc_gState->m_NetworkParams->IsProtocolRightpeer() ? 1 : 100), (int64_t)0);
...
bool CWalletTx::AcceptToMemoryPoolReturnReason(bool fLimitFree, bool fRejectInsaneFee,string& reject_reason)
{
    ...
    {
        pRightPeerFilterEngine->SetTimeout(pRightPeerFilterEngine->GetSendTimeout());
    }
    ...
    if(pRightPeerFilterEngine)
    {
        pRightPeerFilterEngine->SetTimeout(pRightPeerFilterEngine->GetAcceptTimeout());
    }
