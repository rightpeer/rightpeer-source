...
int64_t RightPeerTransaction_OffchainFee(int64_t total_offchain_size);
...
bool FindRelevantCoins(CWallet *lpWallet,                                       // IN  Wallet (to access grouping object)
                       ...
                       unsigned char *in_row,int in_size,                       // TMP temporary buffer row and its size
                       mc_Script *lpScript,                                     // TMP temporary rightpeer script object
                       int *in_special_row,                                     // IN  Coordinates of special rows
                       ...
                       std::string& strFailReason)                              // OUT error message
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(custom_good_for_coin_selection(txout.scriptPubKey) &&
            ...
            ParseRightpeerTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,mapSpecialEntity,strError)))
        {
                        ...
                        std::string& strFailReason)                             // OUT error message
{
    ...
    {       
        ...
        if( (count<max_inputs) && (out.nDepth >= min_conf) )
        {
            ...
            tmp_amounts->Clear();
            if(custom_good_for_coin_selection(txout.scriptPubKey) && 
                    ParseRightpeerTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,strError))
            {
                txout_with_inline_data=false;
                if(check_for_inline_coins)
                {
    ...
    BOOST_FOREACH(const COutput& out, vCoins)                                   // Inputs
    {
        ...
        if(mc_GetABCoinQuantity(in_amounts->GetRow(in_special_row[1]),coin_id))
        {
            ...
            if(mc_GetABCoinQuantity(in_amounts->GetRow(in_special_row[0]),coin_id))
            {
                ...
                if(ParseRightpeerTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,mapSpecialEntity,strError))
                {
                    for(int i=0;i<tmp_amounts->GetCount();i++)
                    {
                      ...
                      const CCoinControl* coinControl)
{
    ...
    if(( nTargetValue > 0) || (!found_selected) )
    {
        ...
        if(!lpWallet->SelectRightPeerCoins(nTargetValue,vCoins,in_map,in_amounts,in_special_row[0],in_asset_row,in_prefered_row,setAssetCoins, nAssetValueIn, coinControl))
        {
            return false;
        }                        
                              ...
                              std::string& strFailReason)                       // OUT error message
{
    ...
    if(MIN_OFFCHAIN_FEE)
    {
        ...
        {
            RightPeerTransaction_SetTmpOutputScript(s.first);
            mc_gState->m_TmpScript->ExtractAndDeleteDataFormat(NULL,NULL,NULL,&total_offchain_size);            
        }
        mandatory_fee=RightPeerTransaction_OffchainFee(total_offchain_size);
    }
...
{   
    ...
    BOOST_FOREACH (const PAIRTYPE(CScript, CAmount)& s, vecSend)
    {
        if (nValue < 0)                                                         // Rightpeer allows protocol zero-value outputs
        {
...
//    if (vecSend.empty() || nValue < 0)
    ...
    if(vecSend.size())
    {
        ...
        if( (addresses == NULL) || (addresses->size() != 1) )
        {
            BOOST_FOREACH (const PAIRTYPE(CScript, CAmount)& s, vecSend)            // Filling buffer with output amounts
            {
                ...
                if(!ParseRightpeerTxOutToBuffer(hash,txout,out_amounts,lpScript,NULL,&this_required,&mapSpecialEntity,strFailReason))
                {
                    goto exitlbl;
                }
    ...
    else
    {
        ...
        if(!ParseRightpeerTxOutToBuffer(hash,txout,out_amounts,lpScript,NULL,&required,strFailReason))
        {
            goto exitlbl;
        }        
    }
    ...
    last_time=this_time;
                                                                                ...
                                                                                // We need this as in SelectRightPeerCoinsMinConf coins will be shuffled
...
bool CWallet::CreateRightPeerTransaction(const vector<pair<CScript, CAmount> >& vecSend,
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl,
...
//    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)                          // Not supported, not used in this case
    ...
    if( (tmp_amounts != NULL) && (lpScript != NULL) )                       // Count assets in unspent coins
    {
        ...
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            ParseRightpeerTxOutToBuffer(hash,txout,tmp_amounts,lpScript,NULL,NULL,strError);
        }
