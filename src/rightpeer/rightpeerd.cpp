...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        fprintf(stderr,"\nError: Data directory %s needs to exist before calling rightpeerd. Exiting...\n\n",mapArgs["-datadir"].c_str());
        delete mc_gState;
        return false;        
    }
    ...
    {
        fprintf(stderr,"\nError: Log directory %s needs to exist before calling rightpeerd. Exiting...\n\n",mapArgs["-logdir"].c_str());
        return false;        
    }
...
*/
        ...
        (mc_gState->m_Params->NetworkName() == NULL))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  rightpeerd <blockchain-name> [options]                     " + _("Start RightPeer Core Daemon") + "\n";
...
//                delete mc_gState;
    ...
    if(err)
    {
        fprintf(stderr,"ERROR: Couldn't read configuration file for blockchain %s. Please try upgrading RightPeer. Exiting...\n",mc_gState->m_Params->NetworkName());
        delete pEF;
        delete mc_gState;                
        return false;
    }
    ...
    if(err)
    {
        ...
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Please restart rightpeerd with reindex=1.\n",mc_gState->m_Params->NetworkName());                        
        }
        else
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Probably rightpeerd for this blockchain is already running. Exiting...\n",mc_gState->m_Params->NetworkName());
        }
        delete pEF;
        delete mc_gState;                
        return false;
    ...
    if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
    {
        ...
        if(mc_gState->IsSupported(protocol_version) == 0) 
        {
            ...
            {
                fprintf(stderr,"ERROR: The protocol version (%d) for blockchain %s has been deprecated and was last supported in RightPeer %s\n\n",
            ...
            {
                fprintf(stderr,"ERROR: Parameter set for blockchain %s was generated by RightPeer running newer protocol version (%d)\n\n",
                        mc_gState->m_Params->NetworkName(),protocol_version);                        
                fprintf(stderr,"Please upgrade RightPeer\n\n");
                delete pEF;
                delete mc_gState;                
                return false;
            }
    ...
    switch(mc_gState->m_NetworkParams->m_Status)
    {
        ...
        case MC_PRM_STATUS_MINIMAL:
            if(mc_gState->GetSeedNode() == NULL)
            {
                ...
                fprintf(stderr,"  rightpeer-util create %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  rightpeer-util clone <old-blockchain-name> %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"\nAnd rerun rightpeerd %s\n\n\n",mc_gState->m_Params->NetworkName());                        
                fprintf(stderr,"If you want to connect to existing blockchain please specify seed node:\n\n");
                fprintf(stderr,"  rightpeerd %s@<seed-node-ip>\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  rightpeerd %s@<seed-node-ip>:<seed-node-port>\n\n\n",mc_gState->m_Params->NetworkName());
                delete pEF;
                delete mc_gState;                
                return false;
            }
        ...
        case MC_PRM_STATUS_INVALID:
            ...
            fprintf(stderr,"  rightpeer-util clone %s <new-blockchain-name>\n",mc_gState->m_Params->NetworkName());
            delete pEF;
            delete mc_gState;                
            return false;
...
/*        
        ...
        if (fDaemon)
        {
            fprintf(stdout, "RightPeer server starting\n");
