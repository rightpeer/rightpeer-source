...
std::string HelpMessageCli()
{
    ...
    strUsage += "  -cold                    " + _("Connect to rightpeerd-cold: use rightpeerd-cold default directory if -datadir is not set") + "\n";
/* MCHN START */    
    strUsage += "  -requestout=<requestout> " + _("Send request to stderr, stdout or null (not print it at all), default stderr") + "\n"; 
    strUsage += "  -saveclilog=<n>          " + _("If <n>=0 rightpeer-cli history is not saved, default 1") + "\n";
/*    
...
//    ParseParameters(argc, argv);
        ...
        mc_gState->m_Params->m_NumArguments<minargs)
      {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  rightpeer-cli <blockchain-name> [options] <command> [params]  " + _("Send command to RightPeer Core") + "\n" +
                  "  rightpeer-cli <blockchain-name> [options] help                " + _("List commands") + "\n" +
                  "  rightpeer-cli <blockchain-name> [options] help <command>      " + _("Get help for a command") + "\n";
...
*/
    ...
    SelectRightPeerBaseParams(mc_gState->m_Params->NetworkName(),RPCPort);
    ...
    if(mc_gState->m_Params->m_NumArguments == 1)                                // Interactive mode
    {
        ...
        while(!exitnow && ((command=term->GetLine()) != NULL))
        {
            ...
            else
            {
                ...
                offset=0;
                argc_p=0;
                strcpy(dest+offset,"rightpeer-cli");
