...
//                *canMine=mc_gState->m_Permissions->CanMine(NULL,pubkey_hash);
                ...
                *canMine=mc_gState->m_Permissions->CanMine(NULL,&pubkey_hash);
                if((*canMine & MC_PTP_MINE) == 0)
                {
                    ...
                    {
                        LogPrintf("mchn: RightPeerMiner: cannot mine now, waiting...\n");
                    }
                    testValidity=false;
                }
                else
                {
                    ...
                    {
                        LogPrintf("CreateNewBlock(): total size %u\n", nBlockSize);
                        LogPrintf("mchn: RightPeerMiner: Starting mining...\n");
                    }                    
                }
            }            
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("RightPeerMiner : mining is paused, generated block is dropped");            
        }
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
        {
            return error("RightPeerMiner : generated block is stale");
        }
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("RightPeerMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("RightPeerMiner started\n");
...
/* MCHN START */            
            ...
            if (Params().MiningRequiresPeers() 
                    ...
                    || (mc_gState->m_NetworkParams->IsProtocolRightpeer() == 0)
                    )
                    ) {
...
/* MCHN END */    
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in RightPeerMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce,pwallet);

            LogPrint("mcminer","mchn-miner: Running RightPeerMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        if(mc_gState->m_ProtocolVersionToUpgrade > mc_gState->m_NetworkParams->ProtocolVersion())
                        if( (mc_gState->m_ProtocolVersionToUpgrade > 0) && (mc_gState->IsSupported(mc_gState->m_ProtocolVersionToUpgrade) == 0) )
                        {
                            LogPrintf("RightPeerMiner: Waiting for upgrade, block is dropped\n");
                        }
                        else
                        {
...
/* MCHN END */    
    ...
    {
        LogPrintf("RightPeerMiner terminated\n");
        throw;
    }
}
