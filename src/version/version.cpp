...
int mc_State::VersionInfo(int version)
{
    ...
    if(version < RIGHTPEER_VERSION_CODE_MIN_VALID)
    {
        switch(version)
        {
            case RIGHTPEER_VERSION_CODE_BUILD:                                 // last version
                return -this_build;                                               
            case RIGHTPEER_VERSION_CODE_PROTOCOL_THIS:                         // last protocol version
                return this_protocol;
            case RIGHTPEER_VERSION_CODE_PROTOCOL_MIN:
                return 10008;                                                   // first supported version
            case RIGHTPEER_VERSION_CODE_PROTOCOL_MIN_DOWNGRADE:
                return 10008;                                                   // cannot downgrade below this version
            case RIGHTPEER_VERSION_CODE_PROTOCOL_MIN_NO_DOWNGRADE:
                return 20002;                                                   // if we are on this version or above, downgrades are forbidden
            case RIGHTPEER_VERSION_CODE_PROTOCOL_FOR_RELEVANCE:                
    ...
    return VersionInfo(RIGHTPEER_VERSION_CODE_BUILD)-1;                        // Created by the following builds
}
...
int mc_State::IsDeprecated(int version)
{
    ...
    if(ptr)
    {
        if(strcmp((char*)ptr,"rightpeer"))
        {
            is_protocol_rightpeer=0;
        }
    }
    
    if(is_protocol_rightpeer)
    {
        if(build > 0)
        {
...
{
    return -VersionInfo(RIGHTPEER_VERSION_CODE_BUILD);
}

int mc_State::GetProtocolVersion()
{
    return VersionInfo(RIGHTPEER_VERSION_CODE_PROTOCOL_THIS);
}

int mc_State::MinProtocolVersion()
{
    return VersionInfo(RIGHTPEER_VERSION_CODE_PROTOCOL_MIN);
}

int mc_State::MinProtocolDowngradeVersion()
{
    return VersionInfo(RIGHTPEER_VERSION_CODE_PROTOCOL_MIN_DOWNGRADE);
}

int mc_State::MinProtocolForbiddenDowngradeVersion()
{
    return VersionInfo(RIGHTPEER_VERSION_CODE_PROTOCOL_MIN_NO_DOWNGRADE);    
}

int mc_State::RelevantParamProtocolVersion()
{
    return VersionInfo(RIGHTPEER_VERSION_CODE_PROTOCOL_FOR_RELEVANCE);        
}
