...
bool RightpeerNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !RightpeerNode_CanConnect(pnode);
    }
...
bool RightpeerNode_AcceptData(CNode *pnode)
{
    ...
    return !RightpeerNode_DisconnectRemote(pnode);
}
...
bool VerifyRightpeerVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!VerifyRightpeerVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(MCP_ANYONE_CAN_CONNECT == 0)
    {
        ...
        if(fIsVerackack)
        {
            ...
            if(!RightpeerNode_CanConnect(pfrom))
            {
