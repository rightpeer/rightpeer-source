...
#include "filters/rightpeerfilter.h"

extern mc_RightPeerFilterEngine* pRightPeerFilterEngine;
...
typedef struct CRightPeerTxDetails
{    
...
bool CRightPeerTxDetails::IsRelevantInput(int vin, int vout)
{
...
bool CRightPeerTxDetails::IsRelevantEntity(uint160 hash)
{
...
void CRightPeerTxDetails::SetRelevantEntity(void *entity)
{
    uint160 hash=0;
...
bool mc_CompareAssetQuantities(CRightPeerTxDetails *details,string& reason)
{
    unsigned char *ptrIn;
...
void RightPeerTransaction_SetTmpOutputScript(const CScript& script1)
{
    CScript::const_iterator pc1 = script1.begin();
    mc_gState->m_TmpScript->Clear();
    mc_gState->m_TmpScript->SetScript((unsigned char*)(&pc1[0]),(size_t)(script1.end()-pc1),MC_SCR_TYPE_SCRIPTPUBKEY);
...
bool RightPeerTransaction_CheckCachedScriptFlag(const CTransaction& tx)
{
    bool flag=false;
...
bool RightPeerTransaction_CheckCoinbaseInputs(const CTransaction& tx,     
                                               CRightPeerTxDetails *details)
{
    if(!tx.IsCoinBase())                                                  
    {   
        return false;
...
bool RightPeerTransaction_CheckInputs(const CTransaction& tx,                  // Tx to check
                                       const CCoinsViewCache &inputs,           // Tx inputs from UTXO database
                                       CRightPeerTxDetails *details,           // Tx details object
...
bool RightPeerTransaction_CheckNewEntity(int vout,
                                          bool& fScriptParsed,
                                          CRightPeerTxDetails *details,   
                                          string& reason)      
{
...
bool RightPeerTransaction_CheckCachedScript(const CTransaction& tx, 
...
void RightPeerTransaction_FillAdminPermissionsBeforeTx(const CTransaction& tx,
                                                        CRightPeerTxDetails *details)      
{
...
bool RightPeerTransaction_VerifyAndDeleteDataFormatElements(string& reason,int64_t *total_size,uint32_t *salt_size)
{    
...
bool RightPeerTransaction_CheckOpReturnScript(const CTransaction& tx, 
                                               const CCoinsViewCache &inputs,   
                                               int vout,
                                               CRightPeerTxDetails *details,   
                                               string& reason)      
{
    ...
    total_offchain_size=0;
    if(!RightPeerTransaction_VerifyAndDeleteDataFormatElements(reason,&total_offchain_size,NULL))
    {
        return false;
    }
    ...
    if( mc_gState->m_TmpScript->GetNumElements() == 2 )                         // Cached input script or new entity
    {
        ...
        {
                                                                                // Cached scripts
            if(!RightPeerTransaction_CheckCachedScript(tx, inputs, fScriptParsed, details, reason))
            {
                return false;
            }
        }
        ...
        {
                                                                                // New entities
            if(!RightPeerTransaction_CheckNewEntity(vout, fScriptParsed, details, reason))
            {
                return false;
            }
        }
                                                                                ...
                                                                                // Performed only if it is indeed needed
    {
        ...
        {
            RightPeerTransaction_FillAdminPermissionsBeforeTx(tx,details);
        }                        
    }
...
bool RightPeerTransaction_CheckAssetUpdateDetails(mc_EntityDetails *entity,
                                                   int vout,
                                                   CRightPeerTxDetails *details,   
                                                   string& reason)      
{
...
bool RightPeerTransaction_CheckUpgradeApproval(const CTransaction& tx,
                                                mc_EntityDetails *entity,
                                                int offset,  
                                                int vout,
                                                CRightPeerTxDetails *details,   
...
bool RightPeerTransaction_CheckStreamItem(mc_EntityDetails *entity,
                                           int vout,
                                           CRightPeerTxDetails *details,   
                                           string& reason)      
{
...
bool RightPeerTransaction_CheckEntityItem(const CTransaction& tx,
                                           int offset,  
                                           int vout,
                                           CRightPeerTxDetails *details,   
                                           string& reason)      
{
    ...
    if(!RightPeerTransaction_VerifyAndDeleteDataFormatElements(reason,NULL,&salt_size))
    {
        return false;
    }
    ...
    {
        if(!RightPeerTransaction_CheckAssetUpdateDetails(&entity,vout,details,reason))
        {
            return false;            
        }
    }
    else
    {
        ...
        {
            if(!RightPeerTransaction_CheckUpgradeApproval(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
        else                                                                    // (Pseudo)stream item
        {
            if(entity.GetEntityType() <= MC_ENT_TYPE_STREAM_MAX)
            {
                ...
                if(!RightPeerTransaction_CheckStreamItem(&entity,vout,details,reason))
                {
                    return false;            
                }            
            }
...
bool RightPeerTransaction_CheckDestinations(const CScript& script1,
                                             int vout,
                                             CRightPeerTxDetails *details,   
                                             string& reason)      
{
...
bool RightPeerTransaction_ProcessPermissions(const CTransaction& tx,
                                              int offset,  
                                              int vout,
...
bool Is_RightPeerLicenseTokenTransfer(const CTransaction& tx)
{
    mc_EntityDetails entity;
    if(tx.IsCoinBase())
    {
    ...
    RightPeerTransaction_SetTmpOutputScript(tx.vout[0].scriptPubKey);
...
bool RightPeerTransaction_CheckLicenseTokenTransfer(const CTransaction& tx,
                                               int unchecked_row, 
                                               CRightPeerTxDetails *details,   
                                               string& reason)      
{
...
bool RightPeerTransaction_CheckAssetTransfers(const CTransaction& tx,
                                               int offset,  
                                               int vout,
                                               CRightPeerTxDetails *details,   
                                               string& reason)      
{
    ...
    if(!RightPeerTransaction_CheckLicenseTokenTransfer(tx,unchecked_row,details,reason))
    {
        return false;
    }
...
bool RightPeerTransaction_CheckOutputs(const CTransaction& tx,                 // Tx to check
                                        const CCoinsViewCache &inputs,          // Tx inputs from UTXO database
                                        ...
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // Basic checks, destinations and simple grants
    {
        ...
        RightPeerTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);

        if(mc_gState->m_TmpScript->IsOpReturnScript())                    
        {
            if(!RightPeerTransaction_CheckOpReturnScript(tx,inputs,vout,details,reason))
            {
                return false;
            }
        }
        ...
        {
            if(!RightPeerTransaction_CheckDestinations(tx.vout[vout].scriptPubKey,vout,details,reason))
            {
                return false;
            }            
            ...
            if(!RightPeerTransaction_ProcessPermissions(tx,offset,vout,permission_type,true,details,reason))
            {
                return false;
            }            
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // create, issue, activate grants (requiring pre-tx permissions)
    {
        if(details->vOutputScriptFlags[vout] & (MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_CREATE | MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_FILTER) )
        {
            ...
            if(!RightPeerTransaction_ProcessPermissions(tx,offset,vout,permission_type,false,details,reason))
            {
                return false;
            }                        
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_OP_RETURN_ENTITY_ITEM)
        {
            ...
            if(!RightPeerTransaction_CheckEntityItem(tx,offset,vout,details,reason))
            {
                return false;                
            }
...
bool RightPeerTransaction_CheckTransfers(const CTransaction& tx,               // Tx to check
                                        int offset,                             // Tx offset in block, -1 if in memppol
                                        CRightPeerTxDetails *details,          // Tx details object
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_NOT_OP_RETURN)
        {
            ...
            if(!RightPeerTransaction_CheckAssetTransfers(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }        
...
int64_t RightPeerTransaction_OffchainFee(int64_t total_offchain_size)            // Total size of offchain items
{
    return (MIN_OFFCHAIN_FEE*total_offchain_size + 999)/ 1000;
}

bool RightPeerTransaction_CheckMandatoryFee(CRightPeerTxDetails *details,     // Tx details object
                                             int64_t *mandatory_fee,            // Mandatory Fee    
                                             string& reason)                    // Error message
{
    *mandatory_fee = RightPeerTransaction_OffchainFee(details->total_offchain_size);
...
bool RightPeerTransaction_CheckLicenseTokenDetails(CRightPeerTxDetails *details,     // Tx details object
...
bool RightPeerTransaction_ProcessAssetIssuance(const CTransaction& tx,         // Tx to check
                                                int offset,                     // Tx offset in block, -1 if in memppol
        ...
        if(details->new_entity_type != MC_ENT_TYPE_ASSET)
        {
            ...
            {                    
                if(mc_gState->m_Features->LicenseTokens())
                {
                    ...
                    RightPeerTransaction_SetTmpOutputScript(tx.vout[issue_vout].scriptPubKey);
                    if(mc_gState->m_Features->License20010())
                    {
                        if(mc_gState->m_TmpScript->GetNumElements() < 2)
                        {
                    ...
                    if(!RightPeerTransaction_CheckLicenseTokenDetails(details,token_address,reason))
                    {
                        return false;                                                                                                                                                        
                    }
...
bool RightPeerTransaction_ProcessEntityCreation(const CTransaction& tx,        // Tx to check
                                                 int offset,                    // Tx offset in block, -1 if in memppol
                                                 bool accept,                   // Accept to mempools if successful
                                                 CRightPeerTxDetails *details, // Tx details object
                                                 string& reason)                // Error message
...
//                if( (details->new_entity_type <= MC_ENT_TYPE_STREAM_MAX) ||     // Admin persmission is required for upgrades and filters
    ...
    if(update_mempool)
    {
        if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&txid))
        {
            ...
            if(details->new_entity_type == MC_ENT_TYPE_FILTER)
            {
                pRightPeerFilterEngine->Add((unsigned char*)&txid+MC_AST_SHORT_TXID_OFFSET,(offset < 0) ? 0 : 1);
            }
        }
        else
        {
...
bool RightPeerTransaction_VerifyNotFilteredRestrictions(const CTransaction& tx,        // Tx to check
                                                         int offset,                    // Tx offset in block, -1 if in memppol
                                                         bool accept,                   // Accept to mempools if successful
                                                         CRightPeerTxDetails *details, // Tx details object
                                                         string& reason)                // Error message
...
bool RightPeerTransaction_VerifyStandardCoinbase(const CTransaction& tx,        // Tx to check
                                                        CRightPeerTxDetails *details, // Tx details object
                                                        string& reason)                // Error message
{
    ...
    if(signature_output == (mc_gState->m_NetworkParams->IsProtocolRightpeer() == 0))
    {
        LogPrintf("Non-standard coinbase: Sigature output doesn't match protocol\n");
        return true;                                                            // Signature output should appear only if protocol=rightpeer
    }
...
bool AcceptRightPeerTransaction   (const CTransaction& tx,                     // Tx to check
                                    const CCoinsViewCache &inputs,              // Tx inputs from UTXO database
                                    int offset,                                 // Tx offset in block, -1 if in memppol
                                    bool accept,                                // Accept to mempools if successful
                                    string& reason,                             // Error message
                                    ...
                                    uint32_t *replay)                           // Replay flag - if tx should be rechecked or only permissions
{
    ...
    if(mc_gState->m_NetworkParams->IsProtocolRightpeer() == 0)                 
    {
        return true;
    }    
    
    details.fCheckCachedScript=RightPeerTransaction_CheckCachedScriptFlag(tx);
    
    mc_gState->m_TmpAssetsIn->Clear();
    
    if(!RightPeerTransaction_CheckCoinbaseInputs(tx,&details))                 // Inputs
    {
        if(!RightPeerTransaction_CheckInputs(tx,inputs,&details,reason))
        {
            return false;
        }
    }
    ...
    if(!RightPeerTransaction_CheckOutputs(tx,inputs,offset,&details,reason))   // Outputs        
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
    
    if(!RightPeerTransaction_ProcessAssetIssuance(tx,offset,accept,&details,reason))                // Asset genesis/followon
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!RightPeerTransaction_CheckMandatoryFee(&details,&mandatory_fee,reason))
    {
        fReject=true;
        goto exitlbl;                                                                            
    }                                                                           

    if(!RightPeerTransaction_CheckTransfers(tx,offset,&details,reason))        // Transfers and receive permissions
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
                                                                                // Creating of (pseudo)streams/upgrades
    if(!RightPeerTransaction_ProcessEntityCreation(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!RightPeerTransaction_VerifyNotFilteredRestrictions(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!RightPeerTransaction_VerifyStandardCoinbase(tx,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    ...
    if( (details.emergency_disapproval_output < 0) && !details.fIsStandardCoinbase && !details.fLicenseTokenTransfer)
    {
        ...
        {
            if(pRightPeerFilterEngine)
            {
                mc_RightPeerFilter* lpFilter;
                int applied=0;

                if(pRightPeerFilterEngine->RunTxFilters(tx,details.vRelevantEntities,reason,&lpFilter,&applied,(offset >= 0)) != MC_ERR_NOERROR)
                {
