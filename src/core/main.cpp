...
bool AcceptRightPeerTransaction(const CTransaction& tx, 
...
bool ProcessRightpeerRelay(CNode* pfrom, CDataStream& vRecv, CValidationState &state);
bool ProcessRightpeerVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool PushRightPeerVerack(CNode* pfrom, bool fIsVerackack);
bool RightpeerNode_CanConnect(CNode *pnode);
bool RightpeerNode_DisconnectRemote(CNode *pnode);
bool RightpeerNode_DisconnectLocal(CNode *pnode);
bool RightpeerNode_RespondToGetData(CNode *pnode);
bool RightpeerNode_SendInv(CNode *pnode);
bool RightpeerNode_AcceptData(CNode *pnode);
bool RightpeerNode_IgnoreIncoming(CNode *pnode);
bool RightpeerNode_IsLocal(CNode *pnode);
bool RightpeerNode_CollectChunks();
...
const string strMessageMagic = "RightPeer Signed Message:\n";
...
int SetUpgradedParamValue(const mc_OneRightpeerParam *param,int64_t value)
{
    ...
    if(strcmp(param->m_Name,"targetblocktime") == 0)
    {
        ...
        SetRightPeerParam("targetblocktime",value);
    }   
...
int RightpeerNode_ApplyUpgrades(int current_height)
{
...
void RightpeerNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool RightpeerNode_IsBlockChainSynced(CNode *pnode)
{
...
/* MCHN START */
        ...
        if(!AcceptRightPeerTransaction(tx,view,-1,true,reason, &mandatory_fee, &replay))
        {
            return state.DoS(0,
                             error("AcceptToMemoryPool: : AcceptRightPeerTransaction failed %s : %s", hash.ToString(),reason),
                             REJECT_NONSTANDARD, reason);
        }
...
*/
        ...
        CAmount nFees = 0;
        int async_count=0;
        bool fIsLicenseTokenTransfer=Is_RightPeerLicenseTokenTransfer(tx);
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            const CTransaction &tx = block.vtx[i];
            string reason;
            if(!AcceptRightPeerTransaction(tx,view,offset,true,reason,NULL,NULL))
            {
...
/* MCHN END */        
    ...
    // See BIP30 and http://r6.ca/blog/201RightPeer Limited6T005236Z.html for more information.
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                ...
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptRightPeerTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
                }
            }
/* MCHN END */                    
...
/* MCHN START */        
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(!AcceptRightPeerTransaction(tx,view,coinbase_offset,true,reason,NULL,NULL))
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptRightPeerTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
...
/* MCHN START */    
    ...
    {
        pRightPeerFilterEngine->Reset(old_height-1,0);
    }
    
    RightpeerNode_ApplyUpgrades(old_height-1);        
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    if(pRightPeerFilterEngine)
    {
        pRightPeerFilterEngine->Reset(pindexNew->nHeight-1,1);
    }
...
/* MCHN START */        
    ...
    RightpeerNode_ApplyUpgrades(chainActive.Height());    
/* MCHN END */    
...
//    if (!fInvalidFound)
    {
        ...
        if(pRightPeerFilterEngine)
        {
            pRightPeerFilterEngine->Reset(chainActive.Height(),0);
        }
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    RightpeerNode_UpdateBlockByHeightList(pindexNew);
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id, CBlockIndex *pindexChecked)
{
    ...
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            RightpeerNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
...
/* MCHN START */
    ...
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
        ...
        if(pwalletTxsMain->RollBack(NULL,chainActive.Height()) != MC_ERR_NOERROR)
        {
            ...
            {
                strError="Error: The wallet database is inconsistent. Restart RightPeer with -rescan to rebuild the wallet database from the blockchain (can take hours), or with -skipwalletchecks to continue operating anyway";
                return false;
            }
        }
    }
...
/* MCHN START */    
    if(!RightpeerNode_RespondToGetData(pfrom)) 
    {
...
//    RandAddSeedPerfmon();
    ...
    if (strCommand == "version")
    {
        ...
        pfrom->nRightPeerServices=0;
        if (!vRecv.empty())
        {
            vRecv >> pfrom->nRightPeerServices;
        }
...
/* MCHN START */
        ...
        {            
            ...
            {
                PushRightPeerVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolRightpeer())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!ProcessRightpeerVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushRightPeerVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolRightpeer())
        {
            ...
            if(!ProcessRightpeerVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !PushRightPeerVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!RightpeerNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !RightpeerNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && RightpeerNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!RightpeerNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && RightpeerNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && RightpeerNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(RightpeerNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
            ...
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && RightpeerNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !RightpeerNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(RightpeerNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=RightpeerNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && RightpeerNode_IsBlockChainSynced(pto))
            {
...
/* MCHN START */        
        ...
        if(pwalletTxsMain->m_ChunkCollector)
        {
            ...
            {
                if(RightpeerNode_CollectChunks())
                {
                    RightpeerCollectChunks(pwalletTxsMain->m_ChunkCollector);
                }                
                pwalletTxsMain->m_ChunkCollector->m_NextTryTimestamp=time_millis_now+RightpeerCollectChunksQueueStats(pwalletTxsMain->m_ChunkCollector);
