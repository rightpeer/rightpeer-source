...
mc_RightPeerFilterEngine* pRightPeerFilterEngine = NULL;
...
/* MCHN START */  
    ...
    {
        delete pRightPeerFilterEngine;
        pRightPeerFilterEngine=NULL;        
    }
...
std::string HelpMessage(HelpMessageMode mode)                                   // MCHN
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "rightpeer.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "rightpeer.pid") + "\n";
#endif
    strUsage += "  -reindex               " + _("Rebuild the blockchain and reindex transactions on startup.") + "\n";
#if !defined(WIN32)
    strUsage += "  -shortoutput           " + _("Returns connection string if this node can start or default rightpeer address otherwise") + "\n";
#endif
/* MCHN START */    
/* Default was 0 */    
...
//    strUsage += "  -blockprioritysize=<n> " + strprintf(_("Set maximum size of high-priority/low-fee transactions in bytes (default: %d)"), DEFAULT_BLOCK_PRIORITY_SIZE) + "\n";
    ...
    strUsage += "\n" + _("RightPeer runtime parameters") + "\n";    
    strUsage += "  -offline                                 " + _("Start rightpeerd in offline mode, no connections to other nodes.") + "\n";
    strUsage += "  -storeruntimeparams                      " + _("Permanently save modifications to runtime parameters made by setruntimeparam APIs.") + "\n";
    strUsage += "  -initprivkey=<privkey>                   " + _("Manually set the wallet default address and private key when running rightpeerd for the first time.") + "\n";
    strUsage += "  -handshakelocal=<address>                " + _("Manually override the wallet address which is used for handshaking with other peers in a RightPeer blockchain.") + "\n";
    ...
    strUsage += "\n" + _("RightPeer API response parameters") + "\n";        
    strUsage += "  -hideknownopdrops      " + strprintf(_("Remove recognized RightPeer OP_DROP metadata from the responses to JSON-RPC calls (default: %u)"), 0) + "\n";
    ...
    strUsage += "  -v1apicompatible       " + strprintf(_("JSON-RPC calls responses compatible with RightPeer 1.0 (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(_("Copyright (c) Coin Sciences Ltd - www.rightpeer.com")) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(_("Full terms are shown at: http://www.rightpeer.com/terms-of-service/")) +
           "\n";
        ...
        if(pwalletMain)
        {
            if(pwalletMain->vchDefaultKey.IsValid())
            {
                ...
                {    
                    ...
                    sprintf(bufOutput,"rightpeer-cli %s grant %s connect\n",mc_gState->m_NetworkParams->Name(),
                         CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                    sprintf(bufOutput,"rightpeer-cli %s grant %s connect,send,receive\n\n",mc_gState->m_NetworkParams->Name(),
                         CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                }
                else
...
/* MCHN END */    
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. RightPeer Core is shutting down."));
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. RightPeer Core is probably already running."), strDataDir));
#endif
/* MCHN END */
...
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolRightpeer())
    {
        LogPrintf("RightPeer version build %s protocol %s (%s)\n", mc_BuildDescription(mc_gState->GetNumericVersion()), mc_gState->GetProtocolVersion(), CLIENT_DATE);
    }
    ...
    if (!fDisableWallet) {
        ...
        if (filesystem::exists(pathWalletDat))
        {
            ...
            if(currentwalletdatversion != 1)
            {
                ...
                {
                    return InitError(strprintf("Wallet version %d is not supported in this edition of RightPeer.\n"
                            "To upgrade to version %d, run RightPeer Offline Daemon: \n"
                            "rightpeerd-cold %s -datadir=%s -walletdbversion=3\n"
                            "and restart rightpeerd or use Community Edition.\n",
        ...
        switch(currentwalletdatversion)
        {
            ...
            case 2:
                break;
            case 1:
                return InitError(strprintf("Wallet version 1 is not supported in this version of RightPeer. "
                        "To upgrade to version 2, run RightPeer 1.0: \n"
                        "rightpeerd %s -walletdbversion=2 -rescan\n",mc_gState->m_NetworkParams->Name()));                                        
            default:
...
//            if (!CWalletDB::Recover(bitdbwrap, strWalletFile, true))
            if(!WalletDBRecover(bitdbwrap,strWalletFile,true))
                return false;
            sprintf(bufOutput,"\nTo work properly with salvaged addresses, you have to call importaddress API and restart RightPeer with -rescan\n\n");
...
//        uiInterface.InitMessage.connect(SetRPCWarmupStatus);
    ...
    uiInterface.InitMessage(_("Initializing rightpeer..."));
    ...
    {    
        ...
        if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
        {
            ...
            return InitError(_("wallet.dat is partially corrupted. Please try running RightPeer with -salvagewallet."));                            
        }
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL)
            {
                ...
                if(init_privkey.size())
                {
                    if(seed_attempt == 1)
                    {
                        ...
                        {
                            return InitError(_("The initprivkey runtime parameter can only be used when connecting to RightPeer 1.0 beta 2 or later"));                                                        
                        }
...
*/
            ...
            if(mc_QuerySeed(seedThreadGroup,seed_resolved.c_str()))
            {
                ...
                {
                    seed_error="Couldn't disconnect from the seed node, please restart rightpeerd";
...
//                    if(seed_port.size() == 0)
                    ...
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d - please check rightpeerd is running at that address and that your firewall settings allow incoming connections.",                
                            seed_ip.c_str(),seed_port);
                        pNodeStatus->sLastError="Couldn't connect to the seed node, rightpeerd is not running or firewall issue";
                    }
                }
            }
            else
            ...
            if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
            {
                ...
                if(mc_gState->IsSupported(protocol_version) == 0) 
                {
                    ...
                    {
                        seed_error=strprintf("The protocol version (%d) for blockchain %s has been deprecated and was last supported in RightPeer %s\n",                
                    ...
                    else
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d.\n"
                                    "Blockchain was created by rightpeerd with newer protocol version (%d)\n"                
                                    "Please upgrade to the latest version of RightPeer or connect only to blockchains using protocol version %d or earlier.\n",                
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
            {
                SelectRightPeerParams(mc_gState->m_Params->NetworkName());
        ...
        else
        {
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_GENERATED)
            {
                ...
                mc_gState->m_NetworkParams->SetGlobals();                           // Needed to update IsProtocolRightpeer flag in case of bitcoin
    ...
    if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
    {
        ...
        {
            seed_error=strprintf("ERROR: Couldn't initialize asset database for blockchain %s. Please restart rightpeerd with reindex=1.\n",mc_gState->m_Params->NetworkName());
...
*/
                ...
                {
                    return InitError("Wallet tx database corrupted. Please restart rightpeerd with -rescan\n");                        
                }
                ...
                {
                    return InitError(strprintf("Wallet tx database was created with different wallet version (%d). Please restart rightpeerd with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_WalletVersion));                        
                }        

                if((pwalletTxsMain->m_Database->m_DBStat.m_InitMode & MC_WMD_MODE_MASK) != (mc_gState->m_WalletMode & MC_WMD_MODE_MASK))
                {
                    return InitError(strprintf("Wallet tx database was created in different mode (%08X). Please restart rightpeerd with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_InitMode));                        
                }        
            }
        }
        ...
        if(pwalletMain == NULL)                                                 // Opening wallet only after rightpeer parameters were initizalized
        {
            pwalletMain = new CWallet(strWalletFile);
            DBErrors nLoadWalletRetForBuild = pwalletMain->LoadWallet(fFirstRunForBuild);
            ...
            if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
            {
                ...
                return InitError(_("wallet.dat corrupted. Please try running RightPeer with -salvagewallet."));                            
            }
...
/* MCHN START */    
    ...
    pRightPeerFilterEngine=new mc_RightPeerFilterEngine;
    if(pRightPeerFilterEngine->Initialize())
    {
    ...
    if(!GetBoolArg("-shortoutput", false))
    {
        if(fListen && !GetBoolArg("-offline",false))
        {
            ...
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            sprintf(bufOutput,"rightpeerd %s:%d\n\n",RightpeerServerAddress(false).c_str(),GetListenPort());
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            if(found_ips > 1)
            {
        ...
        else
        {
            if(GetBoolArg("-offline",false))
            {                
                sprintf(bufOutput,"RightPeer started in offline mode, other nodes cannot connect.\n\n");
                bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));            
            }
            else
            {
    ...
    {
        sprintf(bufOutput,"%s:%d\n",RightpeerServerAddress(true).c_str(),GetListenPort());                
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
    }
...
/* MCHN START */                
                bool fRet = uiInterface.ThreadSafeMessageBox(
                    strLoadError + ".\n\n" + _("Please restart rightpeerd with reindex=1."),
...
//    int version=mc_gState->m_NetworkParams->GetInt64Param("protocolversion");
    ...
    LogPrintf("RightPeer protocol version: %d\n",version);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of RightPeer Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)                         // MCHN
            {
                strErrors << _("Wallet needed to be rewritten: restart RightPeer Core to complete") << "\n";
