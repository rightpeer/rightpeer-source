...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RightPeer
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RightPeer
    // Mac and Unix: ~/.rightpeer
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "RightPeerCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RightPeer";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / ".rightpeer-cold";        
    }
    return pathRet / ".rightpeer";
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"rightpeer",".conf") == 0)
    {
...
int mc_Params::ReadConfig(const char *network_name)
{
    ...
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"rightpeer",".conf"),&mapArgs, &mapMultiArgs,"-");    
...
int mc_RightpeerParams::SetProtocolGlobals()
{
...
int mc_RightpeerParams::SetGlobals()
{
...
int mc_RightpeerParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string RightpeerServerAddress(bool check_external_ip)
{
