...
#include "rightpeer/rightpeer.h"                                              // MCHN
...
/* MCHN START */
static boost::filesystem::path pathCachedRightPeer;
static boost::filesystem::path pathCachedRightPeerLog;
...
/* MCHN START */
    ...
    pathCachedRightPeer=fs::path(string(mc_gState->m_Params->DataDir()));
    fs::path &path =pathCachedRightPeer;
    return path;
...
/* MCHN START */
    fs::path &path =pathCachedRightPeerLog;
    if (!path.empty())
        return path;
    path=fs::path(string(mc_gState->m_Params->DataDir(2,1)));
    return pathCachedRightPeerLog;
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "rightpeer.pid"));
