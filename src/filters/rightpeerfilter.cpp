...
int mc_RightPeerFilterParams::Zero()
{
    m_MaxShownData=-1;
...
int mc_RightPeerFilterParams::Init()
{
    Zero();
...
int mc_RightPeerFilterParams::Close()
{
    mc_gState->m_Compatibility=m_Compatibility;    
    return MC_ERR_NOERROR;
}

int mc_RightPeerFilterParams::Destroy()
{
    return Zero();
}
...
int mc_RightPeerFilter::Destroy()
{
    Zero();
...
int mc_RightPeerFilter::Initialize(const unsigned char* short_txid)
{
    size_t value_size;
    unsigned char *ptr;
    ...
    if(ptr)
    {
        ...
        m_FilterCodeRow=pRightPeerFilterEngine->m_CodeLibrary->GetNumElements();
        pRightPeerFilterEngine->m_CodeLibrary->AddElement();
        pRightPeerFilterEngine->m_CodeLibrary->SetData(ptr,value_size);
        pRightPeerFilterEngine->m_CodeLibrary->SetData(&ntc,1);        
    }                                    
    else
    {    
...
bool mc_RightPeerFilter::HasRelevantEntity(set <uint160>& sRelevantEntities)
{
...
int mc_RightPeerFilterEngine::Zero()
{
    m_Filters.clear();
    m_TxID=0;
    m_Workers=NULL;
...
int mc_RightPeerFilterEngine::Destroy()
{
...
int mc_RightPeerFilterEngine::GetAcceptTimeout()
{    
    return GetArg("-acceptfiltertimeout",DEFAULT_ACCEPT_FILTER_TIMEOUT);
}

int mc_RightPeerFilterEngine::GetSendTimeout()
{    
...
int mc_RightPeerFilterEngine::SetTimeout(int timeout)
{
    for(int i=0;i<(int)m_Filters.size();i++)
    {
        mc_Filter *worker=*(mc_Filter **)m_Workers->GetRow(i);
...
int mc_RightPeerFilterEngine::Add(const unsigned char* short_txid,int for_block)
{    
    int err;
    mc_RightPeerFilter filter;
...
int mc_RightPeerFilterEngine::Reset(int block,int for_block)
{
    int filter_block;
    int err;
...
int mc_RightPeerFilterEngine::NoStreamFilters()
{
...
int mc_RightPeerFilterEngine::RunStreamFilters(const CTransaction& tx,int vout, unsigned char *stream_short_txid,int block,int offset,std::string &strResult,mc_RightPeerFilter **lppFilter,int *applied)            
{
    if(mc_gState->m_Features->StreamFilters() == 0)
    {
        return MC_ERR_NOERROR;
...
int mc_RightPeerFilterEngine::RunTxFilters(const CTransaction& tx,std::set <uint160>& sRelevantEntities,std::string &strResult,mc_RightPeerFilter **lppFilter,int *applied,bool only_once)
{
    int err=MC_ERR_NOERROR;
    strResult="";
    m_Tx=tx;
...
int mc_RightPeerFilterEngine::RunFilter(const CTransaction& tx,mc_Filter *filter,std::string &strResult)
{
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
    m_TxID=m_Tx.GetHash();
...
int mc_RightPeerFilterEngine::RunFilterWithCallbackLog(const CTransaction& tx,int vout,mc_Filter *filter,std::string &strResult, json_spirit::Array& callbacks)
{
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
    m_TxID=m_Tx.GetHash();
...
void mc_RightPeerFilterEngine::SetCallbackNames()
{
    m_CallbackNames.clear();
...
int mc_RightPeerFilterEngine::Initialize()
{
    mc_Buffer *filters;
    unsigned char *txid;
    int err=MC_ERR_NOERROR;
